<template>
  <div class="camera">
    <create-camera :isVisible.sync="createCameraVisible" :cameraGroupID="cameraGroupID" @update-camera-data="handleChange"
      @create-camera-data="handleAdd"></create-camera>
    <update-camera :isVisible.sync="updateCameraVisible" @update-camera-data="handleChange"
      :cameraDataRow="cameraDataRow"></update-camera>
    <set-ai-skill-settings :isVisible.sync="setAISkillSettingsVisible" @update-camera-data="handleChange"
      :cameraDataRow="cameraDataRow"></set-ai-skill-settings>
    <set-extraction-settings :isVisible.sync="setExtractionSettingsVisible" @update-camera-data="handleChange"
      :cameraDataRow="cameraDataRow"></set-extraction-settings>
    <camera-video-preview :isVisible.sync="cameraVideoPreviewVisible" :cameraID="cameraID"></camera-video-preview>
    <div class="create-camera-btn">
      <el-button type="primary" size="medium" @click="handleCreate" icon="el-icon-plus">新建摄像头
      </el-button>
    </div>
    <div class="camera-data-table">
      <el-table :data="cameraData" row-key="camera_id" style="width: 100%;">
        <template v-slot:empty>
          <empty-data>
            <template v-slot:emptyText>
              <div>还没有创建摄像头</div>
              <el-button type="text" @click="handleCreate">马上创建&gt;
              </el-button>
            </template>
          </empty-data>
        </template>
        <el-table-column type="expand">
          <template v-slot="props">
            <el-form label-position="left" label-width="9rem" class="camera-data-detail">
              <el-form-item>
        ...
        <el-table-column prop="camera_id" label="技能ID">
        </el-table-column>
        ...
        <el-table-column prop="add_time" label="最近一次修改时间">
        </el-table-column>
        <el-table-column width="400" label="操作">
          <template v-slot="scope">
            <template v-if="scope.row['state'] === '工作中'">
              <el-button size="small" @click="handleOperation(scope.row['camera_id'], false)" type="text">暂停抽帧
              </el-button>
            </template>
            <template v-else>
              <el-button size="small" @click="handleOperation(scope.row['camera_id'], true)" type="text">开始抽帧
              </el-button>
            </template>
            <el-button size="small" @click="handleExtract(scope.row)" type="text">配置抽帧设置
            </el-button>
            <el-button size="small" @click="handleAISkill(scope.row)" type="text">配置AI技能接口
            </el-button>
            <el-button size="small" @click="handlePreview(scope.row['camera_id'])" type="text">实时预览
            </el-button>
            <el-button size="small" @click="handleUpdate(scope.row)" type="text">修改
            </el-button>
            <el-button size="small" @click="handleDelete(scope.row['camera_id'])" type="text">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      cameraVideoPreviewVisible: false,
      cameraData: [],
      cameraDataRow: null,
      cameraID: ''
    }
  ...
  methods: {
    ...
    handlePreview(cameraID) {
      this.cameraID = cameraID
      this.cameraVideoPreviewVisible = true
    },
